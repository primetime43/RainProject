...
public:
	...
	explicit Setting(const int maxParticles = 10,
		...
		const bool cursorInteraction = true)
		: MaxParticles(maxParticles), WindSpeed(windSpeed), ParticleColor(ParticleColor), PartType(partType), StartWithWindows(startWithWindows), AllowHide(allowHide), CursorInteraction(cursorInteraction)
	{
	}
};
